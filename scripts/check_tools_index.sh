...
JQ_FILTER='del(.generated_at) | . as $o | $o'
...
if ! diff -u <(jq --sort-keys "$JQ_FILTER" "$TMP.orig") <(jq --sort-keys "$JQ_FILTER" "$ORIG") >/dev/null; then
  ...
  diff -u <(jq --sort-keys "$JQ_FILTER" "$TMP.orig") <(jq --sort-keys "$JQ_FILTER" "$ORIG") || true
